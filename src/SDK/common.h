...
_interface* GetInterface(const char* filename, const char* version,
                         bool exact = true) {
#ifdef WIN32
  ...
  if (!result)
    throw std::runtime_error(
        std::string("[get_interface] Failed to GetOffset interface: ") +
        version);
  ...
  for (cur_interface = interfaces; cur_interface;
       cur_interface = cur_interface->m_pNext) {
    if (exact) {
      if (strcmp(cur_interface->m_pName, version) == 0)
        return reinterpret_cast<_interface*>(cur_interface->m_CreateFn());
    } else {
      if (strstr(cur_interface->m_pName, version) &&
          strlen(cur_interface->m_pName) - 3 == strlen(version))
        return reinterpret_cast<_interface*>(cur_interface->m_CreateFn());
    }
  }
